...
extension RowGroup {
    static let bananaSockTop = RowGroup(
        rows: (0..<15).map { _ in Row(instructions: "k2p2 x 14") }
    )

    static let bananaSockRightBody = RowGroup(
        rows: (0..<5).map { _ in Row(instructions: "k2p2 x 7, p2, k26") }
            + (0..<5).map { _ in Row(instructions: "k2p2 x 7, p2, p26") }
    )

    static let bananaSockRightBottom = RowGroup(
